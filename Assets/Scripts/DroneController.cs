...
public class DroneController : MonoBehaviour {
	...
	void FixedUpdate () {
        ...
        //fire weapon
        if ((Time.fixedTime) % 2 == 0&&Vector2.Distance(myRB.position, player.position) < 38&&myRB.position.y>player.position.y&&(int)transform.position.z==(int)playerTransform.position.z)
        {
    ...
    {
        if (col.gameObject.tag == "Player Bullet Small")
        {
